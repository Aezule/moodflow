...
export function computeMonthlyAnalytics(state) {
  ...
  } ${bestDate.toLocaleDateString("fr-FR", {
    weekday: "long",
    day: "numeric",
    month: "long",
  })}`;
...
function getYearMonths(state) {
  ...
  for (let month = 0; month < 12; month++) {
    ...
    if (entries.length > 0) {
      const avg =
        entries.reduce((sum, item) => sum + item.mood, 0) / entries.length;
...
export function computeYearlyAnalytics(state) {
  ...
  const average =
    allEntries.reduce((sum, entry) => sum + entry.mood, 0) / allEntries.length;
  const averageMood = moodLevels[Math.round(average)];
...
function computeWeekAnalytics(state, weekDays) {
  ...
  if (entries.length === 0) {
    return {
      ...
      summary:
        "Ajoutez vos humeurs pour voir un résumé personnalisé de votre semaine.",
...
function computeYearAnalytics(state) {
  ...
  if (entries.length === 0) {
    return {
      ...
      summary:
        "Ajoutez vos humeurs pour voir un résumé personnalisé de l'année.",
  ...
  const average =
    entries.reduce((sum, entry) => sum + entry.averageMood, 0) / entries.length;
  const averageMood = moodLevels[Math.round(average)];
...
function buildChartData(units, entries, labelSelector) {
  ...
  const lineColors = units.map((unit) =>
    unit.entry ? unit.entry.color : "#cccccc"
  );
  ...
  return {
    ...
    bar: {
      ...
      backgroundColors: Object.values(moodLevels).map(
        (level) => `${level.color}80`
      ),
...
const weatherCodeInsights = [
  {
    ...
    modifier: 0.6,
    tooltip:
      "Un ciel dégagé favorise souvent une énergie positive et des activités en extérieur.",
  },
  {
    codes: [1, 2],
    ...
    modifier: 0.3,
    tooltip:
      "Quelques nuages mais assez de lumière pour maintenir un bon niveau de motivation.",
  },
  {
    codes: [3],
    label: "Ciel couvert",
    ...
    modifier: -0.1,
    tooltip:
      "Un ciel gris peut amener un léger ralentissement ou une baisse de moral.",
  },
  {
    codes: [45, 48],
    label: "Brouillard",
    ...
    modifier: -0.2,
    tooltip:
      "La visibilité réduite complique les déplacements et peut fatiguer.",
  },
  {
  ...
  },
  {
    ...
    modifier: -0.4,
    tooltip:
      "Les journées pluvieuses demandent plus d’effort pour maintenir une bonne humeur.",
  },
  {
  ...
  },
  {
    ...
    modifier: -0.6,
    tooltip:
      "Les orages intenses peuvent créer du stress ou limiter les activités extérieures.",
  },
];
...
function buildBaselineFromHistory(state) {
  ...
  const baseline =
    pool.reduce((sum, entry) => sum + entry.mood, 0) / pool.length;
...
export function computePredictionFromForecast(state, forecast) {
  ...
  const baselineMood =
    moodLevels[Math.round(clampMood(baseline))] || moodLevels[3];
  ...
  const points = forecast.days.map((day) => {
    ...
    const weekdayAdjust =
      weekday != null && weekdayModifiers[weekday]
        ? weekdayModifiers[weekday]
        : 0;
  ...
  const topWeather = Object.values(dominantWeather).sort(
    (a, b) => b.count - a.count
  )[0];

  const averageProjectedMood =
    points.reduce((sum, point) => sum + point.mood, 0) / points.length;
  const averageMoodLevel =
    moodLevels[Math.round(clampMood(averageProjectedMood))] || moodLevels[3];
